...
public class ResumeCustomRepositoryImpl implements ResumeCustomRepository {
   ...
   public List<ResumeDTO> viewResumesInMyPage(Long id) {
       return entityManager.createQuery("select new com.f5.resumerry.Resume.dto.ResumeDTO(r.id, r.title, r.contents, r.resumeRecommendList.size,r.resumeCommentList.size,r.views,r.modifiedDate,r.hashtag, m.id, m.imageSrc, m.nickname,r.years ) "
               + "from Resume r "
               + "join r.member m "
