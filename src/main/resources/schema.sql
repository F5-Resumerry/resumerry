...
CREATE TABLE IF NOT EXISTS post (post_id bigint not null auto_increment, created_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP, modified_date TIMESTAMP DEFAULT CURRENT_TIMESTAMP , category varchar(255) not null, contents varchar(255) not null, is_anonymous varchar(255) not null, title varchar(255) not null, views integer not null, member_id bigint references member(member_id), resume_id bigint references resume(resume_id), is_delete varchar(255) not null , primary key (post_id));
